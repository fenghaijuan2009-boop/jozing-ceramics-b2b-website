...
export default function sitemap():MetadataRoute.Sitemap{
  ...
  const products=allProducts.map((product)=>({url:`${origin}/products/${productSlug(product.name)}/`,lastModified:new Date("2026-09-12"),changeFrequency:"weekly" as const,priority:.8}));
  ...
  const categories=stockCategories.filter(c=>c.codes.length).map(c=>({url:`${origin}/stock/${c.slug}/`,lastModified:["ceramic-mug-cup","hot-sale-by-ton-carton","dinner-set-tableware"].includes(c.slug)?new Date("2026-09-12"):now,changeFrequency:"weekly" as const,priority:.7}));
  return[...core,...products,...categories,...articles,...blogs,{url:`${origin}/stock/catalog/`,lastModified:new Date("2026-09-12"),changeFrequency:"weekly",priority:.8}]
}
