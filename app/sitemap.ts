...
export default function sitemap():MetadataRoute.Sitemap{
  ...
  const products=allProducts.map((product)=>({url:`${origin}/products/${productSlug(product.name)}/`,lastModified:new Date(sep18Codes.has(product.code)?"2026-09-18":importedCodes.has(product.code)?"2026-09-14":"2026-09-12"),changeFrequency:"weekly" as const,priority:.8}));
  ...
  const categories=stockCategories.filter(c=>c.codes.length).map(c=>({url:`${origin}/stock/${c.slug}/`,lastModified:c.codes.some(code=>sep18Codes.has(code))?new Date("2026-09-18"):c.codes.some(code=>importedCodes.has(code))?new Date("2026-09-14"):now,changeFrequency:"weekly" as const,priority:.7}));
  return[...core.map(page=>page.url===`${origin}/stock/`||page.url===`${origin}/`?{...page,lastModified:new Date("2026-09-18")}:page),...products,...categories,...articles,...blogs,{url:`${origin}/stock/catalog/`,lastModified:new Date("2026-09-18"),changeFrequency:"weekly",priority:.8}]
}
