...
export default function sitemap():MetadataRoute.Sitemap{
  ...
  const core=["","/about","/contact","/faq","/guides","/stock","/oem-odm","/blog"].map((path,index)=>({url:`${origin}${path}/`,lastModified:now,changeFrequency:index===0?"weekly" as const:"monthly" as const,priority:index===0?1:.7}));
