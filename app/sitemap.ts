...
export default function sitemap():MetadataRoute.Sitemap{
  ...
  const products=allProducts.map((product)=>({url:`${origin}/products/${productSlug(product.name)}/`,lastModified:new Date("2026-09-25"),changeFrequency:"weekly" as const,priority:.8}));
  ...
  const categories=stockCategories.filter(c=>c.codes.length).map(c=>({url:`${origin}/stock/${c.slug}/`,lastModified:(c.codes.includes("JZ-A1601732106162") || buyingPages[c.slug] || c.slug === "ceramic-canister-jar")?new Date("2026-09-25"):c.codes.some(code=>refreshedCodes.has(code))?new Date("2026-09-22"):c.codes.some(code=>sep21Codes.has(code))?new Date("2026-09-21"):c.codes.some(code=>sep18Codes.has(code))?new Date("2026-09-18"):c.codes.some(code=>importedCodes.has(code))?new Date("2026-09-14"):now,changeFrequency:"weekly" as const,priority:.7}));
  return[...core.map(page=>page.url===`${origin}/guides/`?{...page,lastModified:new Date("2026-09-25")}:page.url===`${origin}/oem-odm/`?{...page,lastModified:new Date("2026-09-25")}:page.url===`${origin}/stock/`||page.url===`${origin}/`?{...page,lastModified:new Date("2026-09-25")}:page),...products,...categories,...articles,...blogs,{url:`${origin}/stock/catalog/`,lastModified:new Date("2026-09-25"),changeFrequency:"weekly",priority:.8}]
}
