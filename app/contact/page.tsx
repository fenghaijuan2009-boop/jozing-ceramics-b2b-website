...
export default function ContactPage() { return <main><SiteHeader />
  ...
  <section className="contact-route"><div className="shell"><div><p className="eyebrow">READY STOCK</p><h2>Ask for today&apos;s list.</h2><p>Availability changes quickly. Request current photos, packing, quantity, MOQ, loading information and the best offer for your destination.</p></div><div><p className="eyebrow">OEM / ODM</p><h2>Send your reference.</h2><p>Share your target shape, glaze, pattern, logo, packaging, quantity and market requirements so we can discuss feasibility and the next sampling step.</p></div></div></section>
  <section className="page-cta"><div className="shell"><p className="eyebrow">NEED AN ANSWER FIRST?</p><h2>Review the buyer FAQ.</h2><div><a className="btn primary" href="/faq">Open FAQ</a><a className="btn text" href="/about">About JOZING →</a></div></div></section><SiteFooter /></main>; }
